...
export default function PagamentoPage({
  ...
  searchParams: Promise<{ nome?: string; titulo?: string; erro?: string }>;
}) {
  const { id } = use(params);
  const { nome, erro } = use(searchParams);
  ...
  useEffect(() => {
    if (!gift) router.replace("/lista");
  }, [gift, router]);

  async function handleConfirm() {
    if (!selected || !gift) return;

    if (selected === "pix") {
      router.push(`/lista/${id}/pix?nome=${encodeURIComponent(contributor)}` as Route);
      return;
    }
    ...
    try {
      const res = await fetch("/api/checkout", {
        ...
        body: JSON.stringify({ giftId: id, nome: contributor }),
      });
